...
AGENTIC_SYSTEM_PROMPT = (
    ...
    "- эти инструменты помечены как изменяющие данные и выполнятся только после явного "
    "подтверждения пользователем, но ты всё равно должен предложить их вызвать, если это "
    "именно то, что нужно (не отказывайся сам за пользователя). Отвечай кратко, по-русски, "
    "по делу."
)
